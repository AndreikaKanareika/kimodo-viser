...
class GuiPanelHandle(_GuiHandle[None], GuiPanelProps):
    """Use as a context to place GUI elements into an anchored panel."""
    ...
    def __enter__(self) -> GuiPanelHandle:
        self._container_id_restore = self._impl.gui_api._get_container_uuid()
        self._impl.gui_api._set_container_uuid(self._impl.uuid)
        return self
    ...
    def remove(self) -> None:
        """Permanently remove this panel and all contained GUI elements from the
        visualizer."""
        # Warn if already removed.
        if self._impl.removed:
            warnings.warn(
